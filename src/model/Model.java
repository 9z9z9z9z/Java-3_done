    ...
    public Model() throws IOException {
        ...
        if (!loadUserConfig()){
            ...
            while (flag){
                if (function == 1){
                    if ( !login() ){
                        if(Controller.inputInt("1) Continue entering:\n2)Registration:\n>>>\t") == 2){
                            registration();
                            flag = false;
                        }
                    }
    ...
    public void registration(){
        ...
        while (logFlag && passwordFlag){
            ...
            if (!database.keySet().contains(loginField)){
                usersProp.put(loginField, passwordField);
                usersProp.put(loginField + "_group", grouField);
                configProp.put(loginField, passwordField);
                configProp.put(loginField + "_group", grouField);
                database.put(loginField, passwordField);
                database.put(login+"_group", grouField);
                login = loginField;
